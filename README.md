...
- Passing that check does not guarantee the review runs. A login expires while still reporting as signed in, a plan may not cover codex, and a sandbox that denies writes to `~/.codex` or blocks the network stops codex before it starts. A codex that cannot authenticate may also hang on retries instead of returning the 401. When codex fails for one of those reasons, `codex-reviewer` writes the reason to `/tmp/claude/codex-review-cooldown` and the hook stops asking for a codex review for the next 6 hours, so a broken setup costs one skipped review instead of one per turn. Running `codex login` again ends the wait immediately, because the refreshed credentials are newer than the marker. Set `CLAUDE_CODEX_COOLDOWN_SECONDS` to change the window and `CLAUDE_CODEX_COOLDOWN_FILE` to move the marker.
...
- That write grant is narrowed by `sandbox.filesystem.denyWrite` on `~/.codex/config.toml` and `~/.codex/plugins`. Both name programs codex later runs outside the sandbox — `notify`, `mcp_servers.<name>.command`, and plugin executables — so a sandboxed command that could edit them would be handing itself unsandboxed execution on the user's next codex run. A deny holds inside a wider allow, and codex only needs the rest of the directory.
