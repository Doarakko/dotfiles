...
| Event | Behaviour |
| --- | --- |
| `SessionStart` | Records the worktree so a turn that edits nothing stays quiet, and hands back the review budget when a session truly starts over (`hooks/auto-review-session-start.sh`) |
...
| `Stop` | Reviews uncommitted changes before the turn ends — only on turns that actually edited something, at most `CLAUDE_AUTO_REVIEW_MAX_ROUNDS` times per session (default 5), and scoping every review after the first to the files that changed since the last one (`hooks/auto-review.sh`) |

Wired in `.claude-plugin/plugin.json`. Three things are recorded per session under the temporary directory, never in git — your index and your partial staging stay untouched: where the last review reached, which decides what you are shown; where the last turn ended, which decides whether anything is asked at all; and how many reviews this session has spent. Only a session that genuinely starts over (`startup`, `clear`, `fork`) gets its count back — resuming or compacting carries it. Compaction also leaves the turn marker alone: it happens mid-turn, after edits, so rewriting the marker there would swallow that turn's review. Reaching the per-session cap is announced once and then the hook goes quiet, leaving those changes unreviewed for `/review-diff` rather than marking them done. Whenever a record cannot be trusted (first review of a session, a corrupted or missing record, a path that will not fit on one line, more changed files than `CLAUDE_AUTO_REVIEW_MAX_SCOPE_FILES`, default 40) the hook errs towards asking — falling back to the whole uncommitted diff, or treating the turn as having edited something — rather than silently dropping anything. The decision tables live in `hooks/*.test.sh`, which `.github/workflows/test.yml` runs on pushes to master and on pull requests — read those for the exact behaviour. The same workflow runs `scripts/validate-definitions.sh`, which checks every command, skill, and agent for a closed frontmatter block, for permission specifiers Claude Code accepts but never consults (`Write(path)`, `Glob(path)`, and friends — use `Edit(path)`), and for `doarakko-config:` references that point at nothing. Set `"disableAllHooks": true` in your settings to turn all hooks off, or remove the individual entry from `plugin.json`.
