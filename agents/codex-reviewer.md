...
## 実行できなかったときの待機指示
ログイン済みでもトークンが切れていれば401になる。次のターンで直る類ではないため、繰り返し呼ばれないよう待機の目印を残す。
...
自動レビューhookはこの目印を見て、6時間（`CLAUDE_CODEX_COOLDOWN_SECONDS`で変更可）はcodexレビューを依頼しなくなる。`codex login`で再ログインすると認証情報の更新時刻が目印より新しくなり、待機は即座に打ち切られる（トークンの自動更新など、認証情報が書き換わった場合も同様）。
...
1. レビュー範囲に応じてコマンドを組み立てる。`--sandbox`はサブコマンドより前に置く（グローバルフラグではないため）
   ...
   # 特定のファイルだけに絞る場合も、範囲フラグは使えないのでプロンプトで範囲を伝える
   codex exec --sandbox read-only review "未コミットの変更のうち、次のファイルだけをレビューする: path/a.ts path/b.sh。ほかのファイルの変更は対象外とし、指摘しない。"
   ```
   ...
   - 対象ファイルを渡されたらプロンプト形式を使う。絞り込みを諦めて`--uncommitted`へ戻すと、レビュー済みのファイルまで読み直すことになる
